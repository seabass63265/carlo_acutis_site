...
>(function ImagePlane({ texture, position, scale, material }, ref) {
...
function getAspect(texture: THREE.Texture): number {
...
function GalleryScene({
	...
	// Direct refs to Three.js meshes — updated imperatively in useFrame
	const meshRefs = useRef<(THREE.Mesh | null)[]>(Array.from({ length: visibleCount }, () => null));
	const lastImageIndex = useRef<number[]>(planesData.current.map(p => p.imageIndex));
	...
	useEffect(() => {
		const onWheel = (e: WheelEvent) => {
			velocityRef.current += e.deltaY * 0.01 * speedRef.current;
			autoPlayRef.current = false;
			lastInteractionRef.current = Date.now();
		};
		const onKey = (e: KeyboardEvent) => {
			if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
				velocityRef.current -= 2 * speedRef.current;
				autoPlayRef.current = false;
				lastInteractionRef.current = Date.now();
	...
	useFrame((state, delta) => {
		if (!autoPlayRef.current && Date.now() - lastInteractionRef.current > 2000) {
			autoPlayRef.current = true;
		}

		const autoForce = autoPlayRef.current ? 1.5 * speedRef.current * delta : 0;
		velocityRef.current = (velocityRef.current + autoForce) * 0.92;
		...
		planesData.current.forEach((plane, i) => {
			...
			// Imperatively update mesh position so it moves every frame
			const mesh = meshRefs.current[i];
			if (mesh) {
				mesh.position.set(plane.x, plane.y, plane.z - depthRange / 2);

				// Update texture and scale when image wraps to a different photo
				if (plane.imageIndex !== lastImageIndex.current[i]) {
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full bg-navy-dark p-4">
